#!/usr/bin/env python3
"""
Frontier Detector - Baseline (No Terrain Weighting)
-----------------------------------------------------
Identical to frontier_detector.py except COST_WEIGHT = 0.0, so frontiers
are ranked purely by information gain (frontier size), equivalent to
classic nearest/largest-frontier exploration. Used as the comparison
point against the terrain-weighted version.
"""
...
COST_WEIGHT = 0.0  # baseline: terrain cost disabled, ranks by information gain only


class FrontierDetector(Node):
    def __init__(self):
        super().__init__('frontier_detector_baseline')
        self.subscription = self.create_subscription(
            OccupancyGrid, '/map', self.map_callback, 10)
        self.marker_pub = self.create_publisher(
            MarkerArray, '/frontier_markers', 10)
        self.get_logger().info('Baseline frontier detector started (terrain cost disabled).')
